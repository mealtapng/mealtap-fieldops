...
export default async function AgentsPage() {
  ...
  const captureCounts: Record<string, { total: number; hot_leads: number }> = {}
  for (const r of (capturesResult.data ?? [])) {
    if (!r.captured_by) continue
    if (!captureCounts[r.captured_by]) captureCounts[r.captured_by] = { total: 0, hot_leads: 0 }
    captureCounts[r.captured_by].total++
    if (r.tag === 'hot') captureCounts[r.captured_by].hot_leads++
  }
  ...
  const agents = (agentsResult.data ?? []).map((a: any) => ({
    ...
    hot_leads:          captureCounts[a.id]?.hot_leads ?? 0,
  }))
